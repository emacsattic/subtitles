...
;;              ===================================
;;              ehud@unix.mvs.co.il   972-9-7659599
;;              ===================================
...
returns a list with 3 elements : number, start-time (milliseconds), stop-time"
       (let ((time-rgx "\\(0[0-9]:[0-5][0-9]:[0-5][0-9],[0-9][0-9][0-9]\\)")    ;; HH:mm:ss.nnn
             N strt stop)
...
;; {start-frame}{stop-frame}line1_text|line2_text
  (interactive "NFrames per second (float e.g. 23.976, 25)  ")
       (let ((svd-pos (point-marker))              ;; saved position
           ...
           (goto-char (point-min))
           (while (re-search-forward "^{\\([0-9]+\\)}{\\([0-9]+\\)}\\(.*\\)$" nil t)   ;; search for next
               (setq strt (truncate (* fps (string-to-number (match-string 1)))))
               (setq stop (truncate (* fps (string-to-number (match-string 2)))))
